...
from src.dataset import LBIDRawDataset, YoloTensorDataset, collate_fn
...
def test_model(dataloader: DataLoader, model: YOLOv8, DEVICE, folder='./'):
...
if __name__ == "__main__":
    ...
    if PRETRAINED_MODEL is not None:
        model.load_state_dict(torch.load('./checkpoint/checkpoint4.pth'))

    for epoch in range(20):
        print(f"epoch: {epoch}")
        if TRAIN_MODE:
            optimizer = torch.optim.Adam(model.parameters(), lr=LRS[epoch])
            loss_items = train_model(dataloader, model, optimizer, loss_fn, DEVICE)
            torch.save(model.state_dict(), f"checkpoint/checkpoint{epoch}.pth")
            print(loss_items)
        pred, target, match = test_model(dataloader, model, DEVICE, folder='./checkpoint/')
